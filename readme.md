...
* 異なるオブジェクトにイベントを送信できる他、グループに設定して複数のオブジェクトにイベントを送信できます。◆
* 大量にイベントを発行しても、自動的に全てのイベントを1回の送信にまとめるため、安定して動作します。
* `All`や`Owner`のほかに、`Master`や`playerId`を設定して、イベントの受信者を限定することができます。
...
## 使い方
シーンに`SimpleNetwork.prefab`を配置し、適当なクラスを作って`SimpleNetworkBehaviour`を継承して`SendEvent()`メソッドを実行することで、インスタンス内にいる自分を含むプレイヤーのオブジェクトに値を届けることができます。
...
public class Test : SimpleNetworkBehaviour
{
    void Start()
    {
        SimpleNetworkInit();
...
VRCPlayerApi[] players = GetTargetPlayers(value);
...
// 指定のプレイヤーにイベントを送信
SendEvent(murder.playerID, "Jump", 5.0f);
```
...
## 仕組み
<p align="center"><img alt="structure" src="https://user-images.githubusercontent.com/14051445/219880088-6af5cec6-f50e-4e0b-9e54-75153155920a.png" width="60%"></p>
...
作成されたコードは、専用プロキシの同期変数に投げられ、これにより全てのプレイヤーにコードが同期されます。

`SimpleNetwork`は全てのプロキシを所持しており、全てのプレイヤーが発行したイベントをリアルタイムに受信します。
...
また、プロキシの`UdonBehaviourSyncMode`は`Manual`に設定されており、同期変数は高速に同期されるので、`SimpleNetwork`の`SendEvent()`は、`SendCustomNetworkEvent()`よりも高速な通信を実現しています。

### コード
以下の仕様は、プロキシが通信するコードについて説明しています。
`SimpleNetwork`でどのようなデータが通信されるか知りたい場合は、以下をご覧ください。

![Code](https://user-images.githubusercontent.com/14051445/220130440-ad8d3595-aa9f-4c6c-ad4b-e83cd9703a41.png)
...
* `Source`：送信元オブジェクトID。送信元オブジェクトの管理インデックスを94進数に変換した2桁の94進数です。設定によって桁数を変更でき、デフォルトでは最大8742個のオブジェクトを管理できます。
...
#### 数の圧縮
SimpleNetworkは、通信するコードの文字数を減らすため、94進数とsignedな93進数を利用してコードを圧縮しています。
